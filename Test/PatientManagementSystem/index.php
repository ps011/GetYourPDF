...
if((isset($_GET['pid']))&&($_GET['pid']==01))
{
	...
	$result=mysqli_query($con,"select * from passwords where Username='$username'");
...
if($row)
	{
		...
		{
			$username=$_POST['login'];
			$result=mysqli_query($con,"select * from session where username='$username'");
			$row=mysqli_fetch_array($result);
			...
			else
			{
				...
				mysqli_query("insert into session value ('$username','$sessionid')");
...
?>
<br/>
<h1><center>"Welcome! Hakeem Rafiuddin Ahmed" </center></h1>
...
<a class="button button-primary" href="error.php"><span class="icon icon-right chevron-right">Refresh</span></a>
      </form>
    </div>
